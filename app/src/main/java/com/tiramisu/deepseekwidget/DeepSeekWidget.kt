...
class DeepSeekWidget : AppWidgetProvider() {
    companion object {
        ...
        fun updateWidgets(c:Context,d:WidgetDisplayData){val p=prefs(c);val display=if(d.error!=null)"⚠️ ${d.error}" else d.formattedBalance;p.edit().putString(KEY_CACHED,display).putString(KEY_CACHED_DATA,serialize(d)).apply();val pro=p.getBoolean(KEY_MODEL_PRO,false);val mgr=AppWidgetManager.getInstance(c);for(id in mgr.getAppWidgetIds(ComponentName(c,DeepSeekWidget::class.java))){val v=RemoteViews(c.packageName,R.layout.widget_layout);render(v,d,display,pro,"");v.setOnClickPendingIntent(R.id.widget_container,refreshPi(c));v.setOnClickPendingIntent(R.id.widget_summary_area,refreshPi(c));v.setOnClickPendingIntent(R.id.widget_model_area,modelPi(c));mgr.updateAppWidget(id,v)}}
