...
class DeepSeekWidget : AppWidgetProvider() {
    companion object {
        ...
        private fun serialize(d:WidgetDisplayData)=listOf(d.balance,d.todayCost,d.monthlyCost,d.monthlyTokens,d.updatedAt,d.flashData.totalTokens,d.flashData.cacheHitRate,d.flashData.cost,d.flashData.requests,d.proData.totalTokens,d.proData.cacheHitRate,d.proData.cost,d.proData.requests,d.error?:"").joinToString("|")
        private fun deserialize(raw:String?):WidgetDisplayData? {
            return try {
                ...
                WidgetDisplayData(true, p[0], todayCost = p[1], monthlyCost = p[2], monthlyTokens = p[3].toLong(), updatedAt = p[4].toLong(),
                    flashData = ModelData(p[5].toLong(), p[6], p[7], p[8].toLong()),
                    proData = ModelData(p[9].toLong(), p[10], p[11], p[12].toLong()),
                    error = p[13].ifBlank { null })
            } catch (_: Exception) { null }
        }
        private fun render(v:RemoteViews,d:WidgetDisplayData,display:String,pro:Boolean,status:String="") { v.setTextViewText(R.id.tv_title,"DeepSeek");v.setTextViewText(R.id.tv_updated,if(d.updatedAt>0)d.formattedUpdatedTime else "--:--");v.setTextViewText(R.id.tv_refresh_status,status);v.setTextViewText(R.id.tv_balance_value,display);v.setTextViewText(R.id.tv_today_value,d.formattedTodayCost);v.setTextViewText(R.id.tv_month_value,d.formattedMonthCost);v.setTextColor(R.id.tv_title,BLUE);v.setTextColor(R.id.tv_balance_value,BLUE_DARK);v.setTextColor(R.id.tv_today_value,BLUE_DARK);v.setTextColor(R.id.tv_month_value,BLUE_DARK);val m=if(pro)d.proData else d.flashData;v.setTextViewText(R.id.tv_model,if(pro)"Pro ›" else "Flash ›");v.setTextViewText(R.id.tv_tokens,WidgetDisplayData.formatTokenCount(m.totalTokens));v.setTextViewText(R.id.tv_requests,m.requests.toString());v.setTextViewText(R.id.tv_month_tokens,WidgetDisplayData.formatTokenCount(d.monthlyTokens));v.setTextViewText(R.id.tv_cache,if(m.cacheHitRate=="--")"--" else "${m.cacheHitRate}%");v.setTextViewText(R.id.tv_model_cost,"¥${m.cost}");v.setTextColor(R.id.tv_model_cost,0xFFD39B2C.toInt());val x=m.cacheHitRate.toDoubleOrNull();v.setTextColor(R.id.tv_cache,when{ x==null->0xFF687386.toInt();x>=90->0xFF198754.toInt();x>=80->0xFFE09F00.toInt();else->0xFFD93025.toInt()}) }
        fun updateWidgets(c:Context,d:WidgetDisplayData){val p=prefs(c);val display=if(d.error!=null)"⚠️ ${d.error}" else d.formattedBalance;p.edit().putString(KEY_CACHED,display).putString(KEY_CACHED_DATA,serialize(d)).apply();val pro=p.getBoolean(KEY_MODEL_PRO,false);val priceStatus=DeepSeekPricing.statusLine(DeepSeekPricing.loadCached(c),pro,System.currentTimeMillis());val mgr=AppWidgetManager.getInstance(c);for(id in mgr.getAppWidgetIds(ComponentName(c,DeepSeekWidget::class.java))){val v=RemoteViews(c.packageName,R.layout.widget_layout);render(v,d,display,pro,priceStatus);v.setOnClickPendingIntent(R.id.widget_container,refreshPi(c));v.setOnClickPendingIntent(R.id.widget_summary_area,refreshPi(c));v.setOnClickPendingIntent(R.id.widget_model_area,modelPi(c));mgr.updateAppWidget(id,v)}}
